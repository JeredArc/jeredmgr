...
list_commands() {  # args: none, reads: none, sets: none
	...
	echo -e "   ${BOLD}-q, --quiet${UNBOLD}                 Suppress prompts (for automation)"
	echo -e "   ${BOLD}-f, --force${UNBOLD}                 Force actions without confirmation prompts (use with caution)"
	echo -e "   ${BOLD}-s, --no-status-check${UNBOLD}       Don't retry checking status after starting or stopping a project"
	echo -e "   ${BOLD}-n, --number-of-lines <N>${UNBOLD}   Show N log lines or use 'f' (follow) for 'logs' command (default: follow / for all projects $LOG_LINES)"
}
...
show_help() {  # args: none, reads: none, sets: none
	...
	echo ""
	echo -e "- Look for a ${BOLD}setup.sh${UNBOLD} script in the project path and run it."
	echo ""
	echo -e "- Type '${BOLD}docker${UNBOLD}'"
	echo "   Link the $(format_path "<project-name>.docker-compose.yml") file in the projects directory, chosen in the following order:"
	...
	echo "   - Look for a $(format_path "Dockerfile") in the project directory and create a $(format_path "<project-name>.docker-compose.yml") file in the projects directory from that"
	echo "     with a comment ${ITALIC}${DIM}'# Auto-generated by JeredMgr, will remove images on uninstall'${RESET}"
	echo "   - Otherwise offer to create a Dockerfile in the project path"
	echo ""
	echo -e "- Type '${BOLD}service${UNBOLD}'"
	echo "   Link the $(format_path "<project-name>.service") file in the projects directory, chosen in the following order:"
	...
	echo "   - Otherwise offer to create a $(format_path "<project-name>.service") file in the projects directory"
	...
	echo ""
	echo -e "- Type '${BOLD}docker${UNBOLD}'"
	echo -e "  Run the project with ${BOLD}\`docker compose -f <project-name>.docker-compose.yml --project-directory <project-path> up -d\`${UNBOLD} / ${BOLD}\`... down\`${UNBOLD} etc."
	echo "  (Keep in mind: If a docker project is stopped, it does not automatically start again on reboot,"
	echo "   as ${BOLD}\`docker compose ... down\`${UNBOLD} removes the container. That way, changes to the compose file automatically take effect.)"
	echo ""
	echo -e "- Type '${BOLD}service${UNBOLD}'"
	echo -e "  Run the project with ${BOLD}\`systemctl start <project-name>\`${UNBOLD} / ${BOLD}\`... stop\`${UNBOLD} etc."
	echo "  (Keep in mind: If a service project is stopped, it automatically starts again on reboot,"
	echo "   as ${BOLD}\`systemctl stop ...\`${UNBOLD} does not disable the service. Changes to the service file will only take effect with calling enable again or on a reboot.)"
	echo ""
	echo -e "- Type '${BOLD}scripts${UNBOLD}'"
	echo "  Look for the following scripts in the project path and run them with the corresponding commands:"
	echo -e "  - ${BOLD}start.sh${UNBOLD}"
	echo -e "  - ${BOLD}stop.sh${UNBOLD}"
	echo -e "  - ${BOLD}restart.sh${UNBOLD} (otherwise ${BOLD}start.sh${UNBOLD} + ${BOLD}stop.sh${UNBOLD})"
	echo -e "  - ${BOLD}status.sh${UNBOLD}"
	echo -e "  - ${BOLD}logs.sh${UNBOLD}"
	echo ""
	...
	echo "   If the $(format_path "docker-compose.yml") file has the ${ITALIC}${DIM}'# Auto-generated ...'${RESET} comment, remove all docker images named <project-name>"
	echo ""
	echo -e "- Type '${BOLD}service${UNBOLD}'"
	echo -e "   Delete the service file link from $(format_path "/etc/systemd/system/")"
	echo "   Remove the service with ${BOLD}\`systemctl daemon-reload\`${UNBOLD}"
	echo ""
	echo -e "- Type '${BOLD}scripts${UNBOLD}'"
	echo -e "  Look for a ${BOLD}uninstall.sh${UNBOLD} script in the project path and run it"
	echo ""
	...
	echo ""
	echo -e "- Look for an ${BOLD}update.sh${UNBOLD} script in the project path and run it"
	echo ""
	echo "- Else:"
	...
	echo ""
	echo -e "- The provided project name can contain ${BOLD}+${UNBOLD} as wildcard to match a single project"
	echo ""
	echo -e "- To select a sub directory from a git repository, provide it when creating the project or set the ${BOLD}SUBDIR${UNBOLD} variable in the .env file"
