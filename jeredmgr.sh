...
show_help() {  # args: none, reads: none, sets: none
	...
	echo "   as ${BOLD}\`docker compose ... down\`${RESET} removes the container. That way, changes to the compose file automatically take effect.)"
	echo ""
	echo -e "- Type '${BOLD}service${RESET}'"
	...
	echo "   as ${BOLD}\`systemctl stop ...\`${RESET} does not disable the service. Changes to the service file will only take effect with calling enable again or on a reboot.)"
	echo ""
	...
	echo "   Remove the service with ${BOLD}\`systemctl daemon-reload\`${RESET}"
	echo ""
	echo -e "- Type '${BOLD}scripts${RESET}'"
